...
description: >
  ...
  logical commits, and writes the messages. Delegate commit mechanics here
  instead of spending the main loop's context on them.
...
These hold whether or not the host enforces the `tools` frontmatter — Claude
Code honours it, Cursor and Codex ignore it, so the prose is the contract.
